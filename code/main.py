...
# создание инлайновской клавиатуры
...
# создание реплайновской клавиатуры
## было добавлено кнопку отмены для /new_project, update_projects и skills, нужно было добавить этот код чтобы работала, скорее всего есть вариант получше чем это. ↓                                                                                                                                                     
def gen_markup(rows):
...
# опесание (пока что не сделано в будущем может быть будет готова.)
@bot.message_handler(commands=["discretion"])
def adddiscretion_command(message):
    bot.send_message(message.chat.id, "пока что всего лишь хэндлер")

# создание нового проекта
@bot.message_handler(commands=['new_project'])
def addtask_command(message):
    bot.send_message(message.chat.id, "Введите название проекта:",reply_markup=gen_markup(1))
...
def callback_query(call):
    ...
    ## это сделано чтобы убирало кнопки после ответа, если нет кнопки в сообщение то скипнет
    try:
